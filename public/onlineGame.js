...
//Schachuhr
function setClocks() {
  document.getElementById("timeE").innerHTML = Math.trunc(timeE / 60) + ":" + ((String)(timeE % 60)).padStart(2, "0");
  document.getElementById("timeS").innerHTML = Math.trunc(timeS / 60) + ":" + ((String)(timeS % 60)).padStart(2, "0");
}
    ...
    if (clicked.row.toString() == overlays[i].style.gridRow.charAt(0) && clicked.column.toString() == overlays[i].style.gridColumn.charAt(0)) {
      ...
      for (let j = 0; j < pieces.length; j++) {
        if (pieces[j] == curclicked) {
          ...
          let originalPiece = Object.assign({}, pieces[j]);;
          pieces[j].pos = overlays[i].style.gridColumn.charAt(0) + overlays[i].style.gridRow.charAt(0);
          if (isGameBeginning) {
            intervalID = setInterval(function () {
              console.log(ownColor)
              //obere Zeit
              if (curColor != ownColor) {
                timeE--;
                document.getElementById("timeEPoint").classList.toggle("inactiveTimePoint");
                document.getElementById("timeSPoint").classList.add("inactiveTimePoint");
              }
              //untere Zeit
              else {
                timeS--;
                document.getElementById("timeSPoint").classList.toggle("inactiveTimePoint");
                document.getElementById("timeEPoint").classList.add("inactiveTimePoint");
              }
              setClocks();
              if (timeE <= 0) {
                showWinscreen("White");
          ...
          if (ownColor == "Black") {
            overlays[i].id = overlays[i].id.replaceAll("U", "D");
            overlays[i].id = overlays[i].id.replaceAll("L", "X").replaceAll("R", "L").replaceAll("X", "R");
            originalPiece.pos = (9 - originalPiece.pos.charAt(0)) + "" + (9 - originalPiece.pos.charAt(1));
          }
          console.log(timeE, timeS)
          sendMove(JSON.stringify({ type: "move", moveColor: curColor, move: overlays[i].id, oldPiece: originalPiece, newPiece: pieces[j], clockE: timeE, clockS: timeS }));
...
function getAvailableMoves(piece, fig) {
  ...
  if (piece.color != curColor || ownColor != curColor || ownColor != piece.color) {
    return;
  }
  ...
  for (let i = 0; i < moves.length; i++) {
    if (!moves[i].includes("I")) {
      ...
      drawMoveOverlay(movePos, checkMoveState(movePos, piece), moves[i]);
    } else {
      for (let j = 1; j <= 8; j++) {
        ...
        drawMoveOverlay(movePos, checkMoveState(movePos, piece), moveTemp);
...
function drawMoveOverlay(pos, type, move) {
  if (type == "move") {
    let div = document.createElement("div");
...
function isLegalMove(piece, move, infinite) {
  ...
  if (!infinite) {
    ...
    if (piece.name == "Pawn" && move.includes("2") && piece.pos.charAt(1) != "7") {
      return false;
    }
...
function canRochade() {
  ...
  if (curColor == "White") {
    ...
    if (kingSide) rochadeArray[0] = 1;
    if (queenSide) rochadeArray[1] = 8;

    if (rochadeArray.length == 0) return false;
    return rochadeArray;
  } else {
    if (blackMovedPiecesRochade[0]) return false;
    if (!isFreesquare("28") || !isFreesquare("38") || whiteMovedPiecesRochade[2] || isFieldChecked("28") || isFieldChecked("38")) kingSide = false;
    if (!isFreesquare("58") || !isFreesquare("68") || !isFreesquare("78") || whiteMovedPiecesRochade[1] || isFieldChecked("58") || isFieldChecked("68") || isFieldChecked("78")) queenSide = false;
