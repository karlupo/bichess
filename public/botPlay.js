...
function generateFEN() {
  ...
  return fen.replaceAll("0", "/").substring(2) + " b - " + enPassentChance + " " + halfMoves + " " + movesMade;
}
...
function toChessCorrds(coords) {
  let x = parseInt(coords.charAt(0));
  let y = parseInt(coords.charAt(1));

  let xChar = String.fromCharCode(96 + x);
  let yChar = 9 - y;

  return xChar + "" + yChar;
}

function toNormalCoords(chessCoords) {
  let x = chessCoords.charCodeAt(0) - 96;
  let y = 9 - parseInt(chessCoords.charAt(1));
...
function takePiece(piece) {
  for (let k = 0; k < pieces.length; k++) {
    if (pieces[k].pos == piece.pos && piece != pieces[k]) {
      pieces.splice(k, 1);
      break;
    }
  }
...
async function getBestCounter() {
  const url = "/getbestmove";
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({ fen }), // Formatieren des FEN-Strings als JSON-Objekt
  });

  if (response.ok) {
    const responseData = await response.json();
    for (let i = 0; i < pieces.length; i++) {
      console.log(responseData, toChessCorrds(pieces[i].pos))
      if (toChessCorrds(pieces[i].pos) == responseData.substring(0, 2)) {
        console.log('Piece:', pieces[i])
        pieces[i].pos = toNormalCoords(responseData.substring(2));
        takePiece(pieces[i]);
        curColor = "White";
        drawPieces();
        blocked = false;
